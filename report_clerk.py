...
import pymysql
class report_clerk:
    def __init__(self,clerk_report_window):
        self.home=Toplevel(clerk_report_window)
        ...
        self.hlbl = Label(self.home, text="clerk Details", font=('Clarendon BT', 40), background='#E9C5FF',
    ...
    def fetchalldata(self):
        self.dataconnection()
        try:
            qry = 'select * from clerk_data'
            rowcount = self.curr.execute(qry)
            data = self.curr.fetchall()
            self.pdata = []
            if data:
                for row in data:
                    self.pdata.append(row[:6])
                    self.table1.insert("", END, values=row[:6])
            else:
    ...
    def print(self):
        pdf = my_cust_PDF()
        headings = ['Id', 'Name', 'Phone No','Gender','DOB', 'Address']
        pdf.print_chapter(self.pdata, headings,6)
