...
class ClerkClass:
    ...
    def __init__(self,Clerkwindow):

        self.home=Clerkwindow
        self.home=Toplevel(Clerkwindow)
...
#         Widgets
        self.back = Label(self.home, text="Clerk ", font=('Clarendon BT', 40),
                          background='#E9C5FF', foreground='#550685', relief='groove', borderwidth=5)
        self.l1=Label(self.home,text="Clerk_Unique_ID")
        ...
        self.table1 = Treeview(self.tablearea, columns=['c1', 'c2', 'c3', 'c4', 'c5', 'c6'], height=25)
        self.table1.heading("c1", text="Clerk_Unique_ID")
...
#         Buttons
        ...
        self.imglbl.place(x=x1 + x_diff + x_diff , y=y1+50 , width=150, height=150)
        self.b6.place(x=x1 + x_diff + x_diff , y=y1 + 200, width=150)
        self.clearPage()
        self.home.mainloop()
    ...
    def savedata(self):
        ...
        if self.actualname == self.default_image:
            ...
            self.img1.save("clerk_images//" + self.actualname)
        self.dataconnection()
        try:
            qry = "insert into clerk_data value(%s,%s,%s,%s,%s,%s,%s)"
            rowcount = self.curr.execute(qry, (self.t1.get(), self.t2.get(), self.t3.get(), self.v1.get(),
                                               self.t5.get(), self.t6.get('1.0', END),self.actualname))
            self.conn.commit()
            if (rowcount == 1):
                messagebox.showinfo("Success", "Clerk record Saved successfully", parent=self.home)
                self.clearPage()
        except Exception as e:
    ...
    def Updatedata(self):
        ...
        if self.actualname == self.oldname:
            ...
            if self.oldname == self.default_image:
                ...
                import os
                os.remove("clerk_images//" + self.oldname)
        self.dataconnection()
        try:
            qry=" update clerk_data set Name=%s, Phone=%s, Gender =%s, DOB=%s, Address=%s ,pic=%s where Clerk_Unique_ID=%s"
            rowcount=self.curr.execute(qry,(self.t2.get(),self.t3.get(),self.v1.get(),self.t5.get(),
                                            self.t6.get('1.0',END),self.actualname,self.t1.get()))
            self.conn.commit()
            if (rowcount == 1):
                messagebox.showinfo("Success", "Clerk record Updated successfully", parent=self.home)
            self.clearPage()
        except Exception as e:
    ...
    def Deletedata(self):
        ...
        if ans == "yes":
            ...
            self.dataconnection()
            try:
                qry = " delete from clerk_data  where Clerk_Unique_ID =%s"
                rowcount = self.curr.execute(qry, (self.t1.get()))
                self.conn.commit()
                if (rowcount == 1):
                    messagebox.showinfo("Success", "Clerk Record Deleted Successfully", parent=self.home)
                    self.clearPage()
            except Exception as e:
    ...
    def fetchdata(self,pk=None):
        if pk == None:
            Clerk_Unique_ID = self.t1.get()
        else:
            Clerk_Unique_ID = pk
        self.dataconnection()
        try:
                qry = "select * from clerk_data where Clerk_Unique_ID = %s"
                rowcount = self.curr.execute(qry, (Clerk_Unique_ID))
                ...
                if data:
                    ...
                    self.actualname = data[6]
                    self.oldname = data[6]
                    self.img1 = Image.open("clerk_images//" + self.actualname)
    ...
    def fetchalldata(self):
        ...
        try:
            qry = 'select * from clerk_data where Name like %s '
            ...
            if data:
                ...
                messagebox.showwarning("No clerk data found ", parent=self.home)
        except Exception as e:
    ...
    def validate_check(self):
        if not (self.t1.get().isdigit()):
            messagebox.showwarning("Validation Check", "Invalid Unique id", parent=self.home)
            return False
